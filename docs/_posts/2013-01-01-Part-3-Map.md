...
Looking at `map.py` from [`new-coder/dataviz/lib/tutorial_source/map.py`](https://github.com/econchick/new-coder/blob/master/dataviz/lib/tutorial_source/map.py), you’ll see in lines after the preamble that we’re importing `xml.dom.minidom` which is Python’s [minimal implementation](http://docs.python.org/2/library/xml.dom.minidom.html) of DOM interface, as well as our own module, `parse` as `p`. 
...
* A package is a collection of modules (or packages). A module is one python file, so a package is a collection of python files.
...
To see it up on Google maps, navigate to [maps.google.com](maps.google.com), then click the button “My Places”, then “Create Map”, then “Import”, and select your `file_sf.kml` and upload:
