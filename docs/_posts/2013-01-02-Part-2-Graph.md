...
- Simililar as before, when you open up `graph.py` from [new-coder/dataviz/lib/tutorial_source/graph.py](https://github.com/econchick/new-coder/blob/master/dataviz/lib/tutorial_source/graph.py), you’ll see the language environment setup, as well as an introduction to the module itself. 
...
This is called a list comprehension. You can read it as, “count every dictionary value of every dictionary key set to ‘DayOfWeek’ for every line item in `data_file`.” A list comprehension just a for-loop put in a more elegant, “Pythonic” way. **Challenge yourself:** write out a for-loop for our `counter` variable.
