...
#### For the curious
The differences between `"""docstrings"""` and `# comments` have to do with who the reader will be. Within the a Python shell, if you call `help()` on a particular function or class, it will return the `"""docstring"""` that the developer has written. 
