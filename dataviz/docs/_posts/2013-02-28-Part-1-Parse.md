...
~~~
- What w
e have now is a pretty good skeleton - we know what parameters the function will take (`raw_file` and `delimiter`), what it is supposed to do (our """doc-string"""), and what it will return, `parsed_data`. Notice how the parameters and the return value is descriptive in itself.
...
Continue on to [Part 2]( {{get_url("/Part-2-Graph")}} ) of the tutorial to graph your data with matplotlib and numpy.